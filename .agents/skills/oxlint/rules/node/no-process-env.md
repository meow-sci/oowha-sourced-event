---
title: "node/no-process-env"
rule: "node/no-process-env"
category: "Restriction"
version: "1.23.0"
default: false
type_aware: false
fix: "none"
upstream: "https://github.com/eslint-community/eslint-plugin-n/blob/master/docs/rules/no-process-env.md"
---

| Property | Value |
|----------|-------|
| Category | Restriction |
| Default | no |
| Fix | none |
| Type-aware | no |


### What it does

Disallows use of `process.env`.

### Why is this bad?

Directly reading `process.env` can lead to implicit runtime configuration,
make code harder to test, and bypass configuration validation.

### Examples

Examples of **incorrect** code for this rule:

```js
if (process.env.NODE_ENV === "development") {
  // ...
}
```

Examples of **correct** code for this rule:

```js
import config from "./config";

if (config.env === "development") {
  //...
}
```

## Configuration

This rule accepts a configuration object with the following properties:

### allowedVariables

type: `string[]`

default: `[]`

Variable names which are allowed to be accessed on `process.env`.

## How to use

```json
{
  "rules": {
    "node/no-process-env": "error"
  }
}
```

With options:

```json
{
  "rules": {
    "node/no-process-env": ["error", { /* options */ }]
  }
}
```

## Version

This rule was added in v1.23.0.

## References

- [Upstream rule documentation](https://github.com/eslint-community/eslint-plugin-n/blob/master/docs/rules/no-process-env.md)
