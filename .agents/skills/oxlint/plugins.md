...
For the current status of rule coverage, see the linter [product plan issue](https://github.com/oxc-project/oxc/issues/481). For framework and file type support, see the [compatibility matrix](https://oxc.rs/compatibility).
...
Oxlint focuses on supporting the ecosystem through built-in plugins and ESLint-compatible JavaScript plugins. [Contributions that add rules](https://oxc.rs/docs/contribute/linter/adding-rules) to existing built-in plugins are encouraged.
