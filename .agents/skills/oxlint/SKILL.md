---
name: oxlint
description: skill for using oxlint library for linting and code analysis for various languages
---
...
To make migration simple, Oxlint includes [many rules](./rules.md), with coverage across the linter plugins most teams already use, including:

- ESLint core rules
- TypeScript rules (including type-aware rules)
- Popular plugins such as React, Jest, Vitest, Import, Unicorn, and jsx-a11y
- Custom [JS plugins](./js-plugins.md) compatible with the ESLint plugin ecosystem

This breadth makes migration straightforward without sacrificing rule coverage. And tooling has been built [to migrate your entire linter config for you](./migrate-from-eslint.md).
...
See: [Type-aware linting](./type-aware.md)
...
See: [Multi-file analysis](./multi-file-analysis.md)
...
> **Tip:** If you're migrating from ESLint, see [the "Migrate from ESLint" page](./migrate-from-eslint.md) for detailed guidance.
...
See the [compatibility matrix](https://oxc.rs/compatibility) for detailed framework support.

## Features

- [Native plugins](./plugins.md) for broad rule coverage with hundreds of built-in rules, without a large JavaScript dependency tree.
- [Automatic fixes](./automatic-fixes.md) to apply safe changes quickly.
- [Ignore files](./ignore-files.md) to control which paths are linted.
- [Inline ignore comments](./ignore-comments.md) for ignoring rules within a file.
- [Multi-file analysis](./multi-file-analysis.md) for rules that require project-wide context such as import analysis like [no-cycle](./rules/import/no-cycle.md).
- [Type-aware linting](./type-aware.md) for rules that require TypeScript type information.
- [JS plugins](./js-plugins.md) (**alpha**) for compatibility with existing ESLint plugins.
...
<!-- - [Migrate from Biome](./migrate-from-biome.md) -->
